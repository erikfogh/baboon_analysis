...
# Paths to files
idfile = "/home/eriks/baboondiversity/data/haploidified_chrX_males/idfile_males_8_cluster.ids"
phasefile = "/home/eriks/baboondiversity/data/haploidified_chrX_males/chr{}_v3.phase"
recombfile = "/home/eriks/baboondiversity/data/haploidified_chrX_males/chr{}.males.recombfile"
...
def cp_run_copy(chrom, i_number, cp_dir, em_dir, phasefile, recombfile, label_file, pop_list):
    ...
    {chromopainter} -g {phasefile} -r {recombfile} -t {label_file} -f {pop_list} {i} {i} -i 0 -n $ne -M $mut -o {o_name} -s 1 -j
...
def cp_run_sample(chrom, i_number, pop_dir, em_dir, phasefile, recombfile, label_file, pop_list):
    ...
    {chromopainter} -g {phasefile} -r {recombfile} -t {label_file} -f {pop_list} {i} {i} -i 0 -n $ne -M $mut -o {o_name} -j
...
def summarize_em(i_files, em_dir):
    ...
    for file in  *.EMprobs.done; do tail -1 $file | awk '{{print $3}}' >> ne.txt; done
    for file in  *.EMprobs.done; do tail -1 $file | awk '{{print $4}}' >> mut.txt; done
